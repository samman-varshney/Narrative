...
/**
 * Re-exported from `core/utils/xml.ts`, where it moved when the SEO module
 * began serializing sitemaps. It is still this module's single injection
 * boundary — every title, description, name, tag, URL and identifier passes
 * through it on the way into a feed — the definition simply is not duplicated
 * in two modules that both write XML. See that file.
 */
export { escapeXml } from '../../core/utils/xml';
